...
You can also compile it yourself from source: https://www.openlighting.org/ola/linuxinstall/. It seems to have more extensive features than the apt package, but the package should be enough for what we need.
...
Before patching, you'll need to check which OLA plugins are enabled, because if the needed plugin is disabled (or conflicts with another one), nothing will work. `ola_plugin_info` will give you a list of installed OLA plugins:
...
You see that plugin 13 is enabled, but it's also   in conflict with plugins 5 and 6. So we'll need to disable these two.
...
````
The conflict is still there but once you deactivated the other modules, it shouln't be a problem anymore.
